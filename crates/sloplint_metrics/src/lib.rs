...
pub use aggregate::{
    aggregate, LOGIC_FUNCTION_MIN_COGNITIVE, TOP_LEVEL_MIN_LOGIC, TOP_LEVEL_RATIO_THRESHOLD,
};
pub use file::file_metrics;
// `percentile` is private to `aggregate` but has a focused unit test in this module's `tests`.
#[cfg(test)]
pub(crate) use aggregate::percentile;
